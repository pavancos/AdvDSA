...
public:
    ...
    void insert(HASH<T> *H, T val){
        if(H[hashing(val)].value==-1){
            ...
            C++;
            if(H[hashing(hashing(val)+C*C)].value==-1){
                H[hashing(hashing(val)+C*C)].value=val;
                Count++;
                H->display(H);
                return;
            }
    ...
    void search(HASH<T> *H, T val){
        ...
        for(int i=0;i<M;i++)
            if(H[hashing((val)+coll*coll)].value!=val)
                coll++;
        if (H[hashing(hashing(val)+coll*coll)].value==val)
            cout << "Element of key: " << key <<" is present at: "<<hashing(hashing(val)+coll*coll)<< endl;
        else
